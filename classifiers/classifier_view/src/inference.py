...
if PICASSO:
    BASE_DIR = "/mnt2/fscratch/users/tic_163_colab/pedrofdez/picasso/classifiers/classifier_view"
    DATA_DIR = os.path.join(BASE_DIR, f"data/{NUM_TRAIN}")
    MODEL_PATH = os.path.join(BASE_DIR, f"checkpoints/{MODEL_NAME}-{NUM_TRAIN}_ms")
    ...
    MODEL_PATH = f'D:/classifiers/classifier_view/checkpoints/{MODEL_NAME}-{NUM_TRAIN}'
...
if PRETRAINED:
    MODEL_PATH+='-PT'
if NON_ROTATED:
    #MODEL_PATH+='-NR' Es la otra version
    MODEL_PATH+='-EN'
if NO_AUGMENTATION:
     MODEL_PATH+='-NA'
...
def run_test():
    parser = argparse.ArgumentParser(description="Inference for MRI View Classifier")
    ...
    for subset_key in subsets_to_run:
        ...
        # Load model and weights (Load once outside if multiple subsets, but here for simplicity)
        ...
        for split in splits_to_run:
            ...
            if BASE_OUTPUT_DIR:
                ...
                subset_output_dir = os.path.join("classifiers/classifier_view", subset_key)
            ...
            with open(log_file_path, 'w', encoding='utf-8') as f:
                f.write("=============================================\n")
                f.write(f"    METRICS - VIEW CLASSIFICATION ({MODEL_NAME}) - SUBSET: {subset_key.upper()} - SPLIT: {split.upper()}\n")
                f.write("=============================================\n\n")

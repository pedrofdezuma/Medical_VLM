...
def plot_acc_curve(history,model_name):
    ...
    plt.title(f'Evolución de la Accuracy ({model_name}-{len(history["train_acc"])} épocas)')
    plt.xlabel('Época')
    ...
    save_dir=f"classifiers/classifier_sequence/figures/{model_name}"
...
def plot_learning_curves(history, model_name,num_train,pretrained:bool, non_rotated:bool=False):
    ...
    if len(epochs_range) > 9:
        plt.axvline(x=9, color='red', linestyle='--', label='Descongelación', zorder=1)

    # Set x-ticks step to 2
    plt.xticks(np.arange(0, len(epochs_range), 2))
    ...
    plt.title(f'Evolución de la Accuracy ({model_name})')
    plt.xlabel("Época")
    plt.ylabel("Accuracy")
    plt.legend()
    ...
    # Save
    save_dir = f"classifiers/classifier_sequence/figures/{model_name}-{num_train}"
...
def evaluate_classification(model, dataloader, classes, model_name, num_train, pretrained:bool, device, non_rotated:bool, split:str, plot_cm:bool=True, base_output_dir:str=None):
    """
    ...
    # Check if dataset has samples and paths (for MrIDataset or RegressionSliceDataset)
    ...
    # 2. Calculate matrix (forced size by specifying labels)
    ...
    if plot_cm:
        ...
        plt.title(f'Matriz de Confusión ({split.upper()}) - {model_name}-{num_train}', fontsize=19)
    ...
    report_save_path = os.path.join(report_save_dir, f"{split.upper()}_classification_report.csv")
...
def plot_models_comparison(checkpoint_dirs, model_names, save_dir, pretrained:bool=True, non_rotated:bool=False):
    """
    ...
    plt.tight_layout()
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, f"models_comparison.png")
    plt.savefig(save_path, dpi=300)
    save_path_eps = os.path.join(save_dir, f"models_comparison.eps")
    plt.savefig(save_path_eps, format='eps', dpi=300)
    plt.show()
    return save_path

def plot_models_comparison_full(checkpoint_dirs, model_names, save_dir, pretrained:bool=True, non_rotated:bool=False):
    """
    ...
    plt.tight_layout()
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, f"models_comparison_full.png")
    plt.savefig(save_path, dpi=300)
    save_path_eps = os.path.join(save_dir, f"models_comparison_full.eps")
    plt.savefig(save_path_eps, format='eps', dpi=300)
    ...
    legend_save_path = os.path.join(save_dir, f"models_comparison_full_legend.png")
    fig_leg.savefig(legend_save_path, dpi=300, bbox_inches=bbox.expanded(1.05, 1.05))
    legend_save_path_eps = os.path.join(save_dir, f"models_comparison_full_legend.eps")
...
def plot_time_comparison(checkpoint_dirs, model_names, save_dir, pretrained:bool=True, non_rotated:bool=False):
    """
    ...
    plt.tight_layout()
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, f"time_comparison.png")
    plt.savefig(save_path, dpi=300)
    save_path_eps = os.path.join(save_dir, f"time_comparison.eps")
    plt.savefig(save_path_eps, format='eps', dpi=300)
    plt.show()
    return save_path